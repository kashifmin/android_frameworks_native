...
//    ALOGD("screenshot: result = %s", result<0 ? strerror(result) : "OK");
    ...
    if (true == mPropertiesState.mDumpScreenShot) {
        ...
        String8 fname = String8::format("/data/SF_dump/screenshot_%02d.png", count);

        SkBitmap b;
        b.setConfig(SkBitmap::kARGB_8888_Config, sw, sh);
    ...
    // add debug mark to screenshot buffer
    if (true == mPropertiesState.mLineScreenShot) {
        static uint32_t count = 0;

        uint32_t size = sh * sw * 4 / 32;
        void *ptr = (void *)((uint32_t)(*heap)->getBase() + (size * count));
        ...
        uint32_t minLayerZ, uint32_t maxLayerZ)
{
    ...
    // [MTK] {{{
    // add message to update SF state before screenshot
    // to avoid screenshot timing issue with WMS
    mEventQueue.invalidate(true);
    // [MTK] }}}
