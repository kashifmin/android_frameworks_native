    ...
    mHwc(hwc)
{
    ...
    // [MTK] {{{
    uint32_t usageBits = GRALLOC_USAGE_HW_RENDER | GRALLOC_USAGE_HW_COMPOSER;
    // if hwc < HWC_DEVICE_API_VERSION_1_1, add GRALLOC_USAGE_HW_FB usage bit
    if (!hwc.supportsFramebufferTarget())
        usageBits |= GRALLOC_USAGE_HW_FB;

    mBufferQueue->setConsumerUsageBits(usageBits);
    // [MTK] }}}
