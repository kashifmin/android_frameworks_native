...
status_t HWComposer::createWorkList(int32_t id, size_t numLayers) {
    ...
    if (mHwc) {
        ...
        if (hwcHasApiVersion(mHwc, HWC_DEVICE_API_VERSION_1_1)) {
            ...
            disp.framebufferTarget->ext.mva = 0;
            // [MTK] }}}
        }
...
//        stack.dump("");
    ...
    // [MTK] {{{
    disp.framebufferTarget->ext.connectApi = NATIVE_WINDOW_API_EGL;
    ...
    disp.framebufferTarget->ext.mva = buf->getMva();
    if (id == HWC_DISPLAY_PRIMARY) {
        mNeedWaitFBT = false;
        mFBTCondition.signal();
    }
