...
/* fix err PTR when count=0  -- by liaoxl.lenovo 1.22.2013 start */
        ...
        {
        	/* zero event returned, retry polling */
        	//ALOGD("SensorService::threadLoop poll count=0");
        }
        else
        {
	        recordLastValue(buffer, count);
...
#endif
/* fix err PTR when count=0  -- by liaoxl.lenovo 1.22.2013 end */
        ...
        int handle)
{
    ...
    if (rec) {
        ...
        if (rec->removeConnection(connection)) {
			...
			/* clr mLastEventSeen data buffer to fix send error event when reenable sensor -- by liaoxl.lenovo 9.22.2013 start */
			sensors_event_t event;
			memset(&event, 0, sizeof(event));
			mLastEventSeen.editValueFor(handle) = event;
			/* clr mLastEventSeen data buffer to fix send error event when reenable sensor -- by liaoxl.lenovo 9.22.2013 end */
        }
        SensorInterface* sensor = mSensorMap.valueFor(handle);
