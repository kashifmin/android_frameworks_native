...
void GraphicBuffer::free_handle()
{
    if (mOwner == ownHandle) {
        // [MTK] {{{
        XLOGD("close handle(%p) (w:%d h:%d f:%d)",
        ...
        // [MTK] {{{
        XLOGI("free buffer (w:%d h:%d f:%d) handle(%p)",
        ...
        uint32_t reqUsage)
{
    ...
    // [MTK] {{{
    XLOGI("allocate buffer (w:%d h:%d f:%d) handle(%p) err(%d)",
        ...
        int fds[], size_t count)
{
    ...
    if (numFds || numInts) {
        ...
        handle = h;
        // [MTK] {{{
        XLOGD("create handle(%p) (w:%d, h:%d, f:%d)",
            handle, stride, height, format);
        // [MTK] }}}
    } else {
